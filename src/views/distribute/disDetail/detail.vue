...
<!--      </div>-->
      <div class="dis-wrapper">
        <div class="line1">
          <div class="title">{{mock.goodsinfo.title}}</div>
          <div class="collect">
            <p class="icons" v-if="1"><svg-icon icon-class="like"></svg-icon></p>
            <p class="icons" v-if="0"><svg-icon icon-class="like_fill"></svg-icon></p>
            <p class="tip">收藏</p>
          </div>
        </div>
        <div class="price"><span>￥{{mock.goodsinfo.price}}</span> <b>库存商量：100</b></div>
        <div class="post">快递： {{mock.goodsinfo.fastfree}}</div>
      </div>
      <!--    优惠券-->
...
<script>
  ...
  export default {
    ...
    beforeRouteLeave(to, from, next) {
      if (to.path =='/distribute/disOrder') {
    ...
    methods: {
      ...
      buy() {
        if (this.checkInfo()) {
          this.$router.push('/distribute/disOrder')
        }
      },
      addToCart() {
        if (this.checkInfo()) {
